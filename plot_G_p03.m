...
if ~isempty(first_zeros1)
    plot(first_zeros1(:,1), first_zeros1(:,2), 'r-', 'LineWidth', 2); % The solid line indicates the first value with two zeros
end
if ~isempty(second_zeros1)
    plot(result1(:,1), result1(:,2), 'r--', 'LineWidth', 2); % The dotted line represents the value with only one zero versus the second value with two zeros
end

if ~isempty(first_zeros2)
    plot(first_zeros2(:,1), first_zeros2(:,2), 'b-', 'LineWidth', 1); % The solid line indicates the first value with two zeros
end
if ~isempty(second_zeros2)
    plot(result2(:,1), result2(:,2), 'b--', 'LineWidth', 1); % The dotted line represents the value with only one zero versus the second value with two zeros
end
...
plot([3, first_zeros1(end,1)], [0, 0], 'r--', 'LineWidth', 2.5, 'HandleVisibility','off');
plot([first_zeros1(end,1), 12], [0, 0], 'r-', 'LineWidth', 2.5, 'HandleVisibility','off');

plot([3, first_zeros2(end,1)], [0, 0], 'b--', 'LineWidth', 1.5, 'HandleVisibility','off');
plot([first_zeros2(end,1), 12], [0, 0], 'b-', 'LineWidth', 1.5, 'HandleVisibility','off');
